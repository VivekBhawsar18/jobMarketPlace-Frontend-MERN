...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    if (viewMode === "summary") {
      ...
      fetchCategory(viewMode, 1, search);
    }
  }, [viewMode]);

  const fetchCategory = async (mode, page, query) => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/${mode}?page=${page}&limit=10&search=${query}`);
      setCatData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const toggleJS = async (id, isActive) => {
    if (isActive !== false && !window.confirm("Are you sure you want to suspend this job seeker?")) return;
    await api.patch(`/admin/jobseekers/${id}/toggle-active`);
    if (viewMode === "summary") load();
    else fetchCategory(viewMode, catData.page, search);
  };

  const toggleEmp = async (id, isActive) => {
    if (isActive !== false && !window.confirm("Are you sure you want to suspend this employer?")) return;
  ...
  return (
    <main style={{ width: '95%', margin: '2rem auto' }}>
      ...
      {viewMode === "summary" ? (
          ...
          <section style={{ marginTop: '3rem' }}>
            ...
            <div className="table-container">
              <table>
                ...
                <tbody>
                  {jobseekers.length === 0 ? (
                    <tr><td colSpan="6" style={{ textAlign: 'center', color: '#64748b', padding: '1.5rem' }}>No job seekers registered</td></tr>
                  ) : jobseekers.slice(0, 3).map((u) => (
                    <tr key={u._id}>
                      <td style={{ color: '#64748b', fontSize: '0.85rem' }}>{u._id.slice(-4)}</td>
                      <td style={{ fontWeight: 600 }}>{u.name}</td>
                      <td>{u.email}</td>
                      <td><span className={`badge ${u.isVerified ? 'badge-success' : 'badge-info'}`}>{u.isVerified ? 'Verified' : 'Unchecked'}</span></td>
                      ...
                      <td>
                        <div className="action-btns">
                          ...
                          <button className="admin-btn" style={{ background: u.isActive === false ? '#10b981' : '#f43f5e' }} onClick={() => toggleJS(u._id, u.isActive)}>
          ...
          <section style={{ marginTop: '3rem' }}>
            ...
            <div className="table-container">
              <table>
                ...
                <tbody>
                  {employers.length === 0 ? (
                    <tr><td colSpan="8" style={{ textAlign: 'center', color: '#64748b', padding: '1.5rem' }}>No employers registered</td></tr>
                  ) : employers.slice(0, 3).map((c) => (
                    <tr key={c._id}>
                      ...
                      <td><span className={`badge ${c.isActive === false ? 'badge-warning' : 'badge-success'}`}>{c.isActive === false ? 'Suspended' : 'Active'}</span></td>
                      <td>
                        <div className="action-btns">
                          <button className="admin-btn" onClick={() => alert("Edit Employer " + c._id)}>Edit</button>
                          <button className="admin-btn" style={{ background: c.isActive === false ? '#10b981' : '#f43f5e' }} onClick={() => toggleEmp(c._id, c.isActive)}>
        ...
        <section style={{ marginTop: '2rem', marginBottom: '4rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
            <form onSubmit={onSearch} style={{ display: 'flex', gap: '0.5rem', flex: 1 }}>
              <input 
                type="text" 
            ...
            <button className="admin-btn-outline" onClick={() => fetchCategory(viewMode, 1, search)}>↻ Refresh</button>
          </div>
            ...
            {loading ? (
              ...
              <table>
                <thead>
                  {viewMode === "jobseekers" && (
                    <tr>
                      <th>ID</th><th>Name</th><th>Email</th><th>Verification</th><th>Status</th><th>Actions</th>
                    </tr>
                  )}
                  {viewMode === "employers" && (
                    <tr>
                      <th>ID</th><th>Company</th><th>Contact</th><th>Email</th><th>Verification</th><th>Status</th><th>Actions</th>
                    </tr>
                  )}
                  {viewMode === "admins" && (
                    <tr>
                  ...
                  {viewMode === "jobs" && (
                    <tr>
                      <th>ID</th><th>Title</th><th>Employer</th><th>Status</th><th>Date</th><th>Actions</th>
                    </tr>
                  )}
                  {viewMode === "bids" && (
                    <tr>
                      <th>ID</th><th>Job</th><th>Seeker</th><th>Amount</th><th>Date</th><th>Actions</th>
                    </tr>
                  )}
                </thead>
                <tbody>
                    ...
                    <tr key={item._id}>
                      {viewMode === "jobseekers" && (
                        <>
                          ...
                          <td>{item.email}</td>
                          <td><span className={`badge ${item.isVerified ? 'badge-success' : 'badge-info'}`}>{item.isVerified ? 'Verified' : 'Unchecked'}</span></td>
                          <td><span className={`badge ${item.isActive === false ? 'badge-warning' : 'badge-success'}`}>{item.isActive === false ? 'Suspended' : 'Active'}</span></td>
                          <td>
                            <div className="action-btns">
                              <button className="admin-btn" style={{ background: item.isActive === false ? '#10b981' : '#f43f5e' }} onClick={() => toggleJS(item._id, item.isActive)}>
                                {item.isActive === false ? 'Reactivate' : 'Suspend'}
                              </button>
                            </div>
                          </td>
                        </>
                      ...
                      {viewMode === "employers" && (
                        <>
                          ...
                          <td>
                            <div className="action-btns">
                              <button className="admin-btn" style={{ background: item.isActive === false ? '#10b981' : '#f43f5e' }} onClick={() => toggleEmp(item._id, item.isActive)}>
                                {item.isActive === false ? 'Reactivate' : 'Suspend'}
                              </button>
                            </div>
                          </td>
                      ...
                      {viewMode === "admins" && (
                        <>
                          ...
                          <td>{item.email}</td>
                          <td><span className={`badge ${item.isActive === false ? 'badge-warning' : 'badge-success'}`}>{item.isActive === false ? 'Suspended' : 'Active'}</span></td>
                          <td><button className="admin-btn" onClick={() => alert("Edit Admin " + item._id)}>Edit</button></td>
                        </>
                      )}
                      {viewMode === "jobs" && (
                        <>
                          ...
                          <td>{item.employerId?.companyName || "N/A"}</td>
                          <td><span className={`badge ${item.status === 'open' ? 'badge-success' : 'badge-info'}`}>{item.status}</span></td>
                          <td>{new Date(item.createdAt).toLocaleDateString()}</td>
                          <td>
                            <div className="action-btns">
                              <button className="admin-btn" style={{ background: '#10b981' }} onClick={() => updateJobStatus(item._id, "open")}>Approve</button>
                              <button className="admin-btn" style={{ background: '#f43f5e' }} onClick={() => updateJobStatus(item._id, "rejected")}>Reject</button>
                            </div>
                          </td>
                        </>
                      )}
                      {viewMode === "bids" && (
                        <>
                          <td>{item._id.slice(-4)}</td>
                          <td>{item.jobId?.description?.slice(0, 30)}...</td>
                          <td>{item.jobSeekerId?.name}</td>
                          <td>${item.bidAmount || 0}</td>
                          <td>{new Date(item.createdAt).toLocaleDateString()}</td>
                          <td><button className="admin-btn" onClick={() => showDetail(item)}>Details</button></td>
                        </>
                      )}
                    </tr>
                  ))}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1.5rem', marginTop: '2rem' }}>
            <button 
              ...
              disabled={catData.page <= 1 || loading}
              onClick={() => fetchCategory(viewMode, catData.page - 1, search)}
            >
              Previous
            </button>
            <span style={{ fontWeight: 600, color: '#475569' }}>
              Page {catData.page} of {Math.ceil(catData.total / 10) || 1}
            </span>
            <button 
              className="admin-btn-outline" 
              disabled={catData.page >= Math.ceil(catData.total / 10) || loading}
              onClick={() => fetchCategory(viewMode, catData.page + 1, search)}
            >
              Next
            </button>
